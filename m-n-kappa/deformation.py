...
class MKappaCurvesAlongBeam: 
	...
	__slots__ = '_crossection', '_beam_length', '_elements', '_m_kappa_curves', '_loading'
	
	def __init__(self, crosssection:section.Crosssection, beam_length:float, elements:int=10, loading_type:str='uniform'): 
	...
	def _compute_m_kappa_curves(self) -> list: 
		"""compute all M-Kappa-Curves"""
			return [{'position':position, 'curve': self._m_kappa(position)} for position in self.positions()]	
	...
	def _m_kappa(self, position:float): 
		...
		return points.MKappaCurve(self.crosssection, position).m_kappa_points # Todo: position
...
class BeamCurvatures:
	...
	def __init__(self, m_kappa_curves:list): 
		...
		self._beam_length = max(self.m_kappa_curves, key=lambda x: x['position'])['position']
	...
	def _curvatures_at_supports(self) -> list: 
		...
		return [{'position':0., 'curvature':0.}, {'position':self.beam_length, 'curvature':0.}]
	
	def _curvatures_within_supports(self, load:float) -> list: 
		"""curvatures inside beam"""
		return [{'position':m_kappa['position'], 'curvature':self._curvature(m_kappa['position'], load)} for m_kappa in self.m_kappa_curves]
	
	
	def _moment(self, position, load): 
		"""compute moment at given position under given load"""
		return internalforces.SingleSpanUniformLoad(self.beam_length, load).moment(position)
...
class DeformationByCurvatures: 
	...
	def deformations(self): 
		...
		return [{'position':position, 'deformation':self.deformation(position)} for position in self.positions]
	...
	def _incremental_deformations(self, position:float): 
		...
		return [self._incremental_deformation(index, moment_curvature) for index in range(self.element_number-1)]
	...
	def _moments_times_curvatures(self, position:float): 
		...
		return [m_curvature[0]*m_curvature[1] for m_curvature in zip(moments, self.curvatures)]
		
	def _single_load(self, position:float) -> internaforces.SingleSpanSingleLoads: 
		"""internal forces with load at given position"""
		return internalforces.SingleSpanSingleLoads(self.beam_length, [position, 1.])
