...
class General(Function): 
	...
	def _print_function(self) -> str: 
		return general.print_sections(['Function', '--------', 'f(x) = ' + self._function_repr()])
		
	def _print_derivate(self) -> str: 
		return general.print_sections(['Derivate', '--------', "f'(x) = " + self._derivate_repr()])
		
	def _print_integration(self) -> str: 
		return general.print_sections(['Integration', '-----------', 'F(x) = ' + self._integration_repr()])
...
class Linear(General):
	...
	def integration(self, variable_value:float): 
		return (
			(1. / 2.) * self.slope * variable_value ** (2.) + 
...
class Polynominal(General):
	...
	def derivate(self, variable_value:float): 
		return (
			2. * self.a * variable_value + 
			self.b)
		
	def function(self, variable_value:float): 
		return (
			self.a * variable_value ** (2.) +
			self.b * variable_value + 
			self.c)
		
	def integration(self, variable_value:float, constant:float=0.0): 
		return (
			(1./3.) * self.a * variable_value ** (3.) + 
			(1./2.) * self.b * variable_value ** (2.) + 
	...
	def _integration_repr(self): 
			return '(1/3) {:.2f} x^3 + (1/2) {:.2f} x^2 + {:.2f} x'.format(self.a, self.b, self.c)
	...
	def __set_a(self): 
		self._a = (
			(self.y[2]-self.y[0])/((self.x[2]-self.x[0])*(self.x[2]-self.x[1])) -
			(self.y[1]-self.y[0])/((self.x[1]-self.x[0])*(self.x[2]-self.x[1])))
			
	def __set_b(self): 
		self._b = (
			((self.y[1]-self.y[0])/(self.x[1]-self.x[0]))-(self.x[1]+
			self.x[0])*self.a)
