...
class Material: 
	...
	def _print_title(self) -> str: 
		...
		return general.print_sections([class_name, len(class_name)*'=', 'section-type: ' + self.section_type])
	...
	def _print_stress_strain_points(self) -> str: 
		return general.print_sections([' {:9.2f} | {:9.5f}'.format(point[0], point[1]) for point in self.stress_strain])
	
	def __eq__(self, other): 
		return self.stress_strain == other.stress_strain and self.section_type == other.section_type
	...
	def get_intermediate_strains(self, strain_1:float, strain_2:float=0.0) -> list: 
		...
		min_index, max_index = self._order_material_indizes(material_index_2, material_index_1)
	...
	def _get_material_index(self, strain:float):
		...
		for material_index in range(len(self.stress_strain)-1): 
			if self.stress_strain[material_index][1] <= strain < self.stress_strain[material_index+1][1]:
...
class ConcreteCompressionNonlinear(ConcreteCompression): 
	...
	@property
	def cu(self) -> float:
		return 0.001 * min((2.8 + 27.0 * ((98.0 - float(self.f_cm)) / 100.0) ** (4.0)), 3.5) 
	
	@property 
	def strains(self) -> list: 
		return [
			self.yield_strain, 0.33*(self.yield_strain+self.c), 
			0.66*(self.yield_strain+self.c), self.c, 
			0.5*(self.c+self.cu), self.cu]	
...
class ConcreteCompressionParabolaRectangle(ConcreteCompression): 
	...
	@property
	def cu(self) -> float: 
		return 0.001 * min((2.6 + 35.0 * ((90.0 - float(self.f_ck)) / 100.0) ** (4.0)), 3.5)
...
class ConcreteTension: 
	...
	def __init__(self, f_cm:float, E_cm:float, f_ctm:float=None, use_tension:bool=True): 
		self._f_cm = f_cm
		self._E_cm = E_cm
		self._f_ctm = f_ctm
...
class Concrete(Material): 
	...
	def __init__(self, f_cm:float, f_ctm:float=None, use_tension:bool=True, compression_stress_strain_type:str='Nonlinear', tension_stress_strain_type='Default'):
		self._f_cm = f_cm
		self._f_ctm = f_ctm
		self._use_tension = use_tension
	...
	def _print_elastic_values(self) -> str: 
		return general.print_sections([
			'Elastic', '-------', 'E_cm = {:.1f} N/mm^2, | epsilon_y={:.4f}'.format(self.E_cm, self.epsilon_y)
			])
	...
	def _print_compressive_values(self) -> str: 
		text = [			
			...
			'epsilon_c = {:.4f} | epsilon_cu = {:.4f}'.format(self.epsilon_c, self.epsilon_cu)]
		return general.print_sections(text)
	...
	def __set_compression(self):
		...
		if type.upper() == 'NONLINEAR': 
			...
			return ConreteCompressionParabolaRectangel(self.f_cm, self.epsilon_y, self.E_cm)
		elif type.upper() == 'BILINEAR': 
			return ConcreteCompressionBilinear(self.f_cm, self.epsilon_y, self.Ecm)
		else:
			raise ValueError(str(type) + ' not a valid value. Valid values are "Nonlinear", "Parabola" or "Bilinear"')
	...
	def __set_tension(self) -> ConcreteTension: 
		...
		if type.upper() == 'DEFAULT': 
			...
			raise ValueError(str(type) + ' is not a valid value. Valid value is "Default"')
...
class Steel(Material):
	...
	def __init__(self, f_y : float, f_u : float = None, epsilon_u : float = None, E_a : float = 210000): 
		self._f_y = f_y
		self._f_u = f_u
	...
	def __str__(self): 
		text = [
			self.class_name, len(self.class_name)*'=', 
		...
		if not self.__is_elastic: 
			...
			text.append('Plastic')
			text.append('-------')
			text.append('f_y: {:.1f} N/mm^2 | epsilon_y: {:.5f}'.format(self.f_y, self.epsilon_y))
		if self.__is_plastic: 
			text.append('f_u: {:.1f} N/mm^2 | epsilon_u: {:.5f}'.format(self.f_u, self.epsilon_u))
		text +=[
			'', 
			'Stress-Strain-Relationship', '--------------------------', 
