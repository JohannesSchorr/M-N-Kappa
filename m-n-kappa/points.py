...
class MKappa: 
	...
	def __init__(self, cross_section:section.Crosssection, applied_axial_force:float=0.0, maximum_iterations:int=10, axial_force_tolerance:float=5.0, solver:solver.Solver=solver.Newton): 
		"""
		Initialization
		...
		self._computations = [] # [iteration, computed_cross_section, curvature, neutral_axis, horizontal_forces]
		self._solver = solver
	...
	def _print_iterations(self): 
		...
		for iteration in self.computations: 
			text.append('{:4} | {:9.6f} | {:12.2f} | {:10.2f}'.format(iteration['iteration'], iteration['curvature'], iteration['neutral_axis'], iteration['axial_force']))
		text.append('---------------------------------------------')
	...
	def _guess_neutral_axis(self): 
		...
		return self.solver(
			data=self._computations, 
			target='axial_force', 
			variable='neutral_axis').compute()
...
class MKappaByStrainPosition(MKappa): 
	...
	__slots__ = '_cross_section', '_strain_position', '_strain_at_position', '_applied_axial_force', '_axial_force_tolerance', '_maximum_iterations', '_maximum_curvature', '_minimum_curvature', '_computations', '_solver', '_successful', '_computed_cross_section', '_iteration'
	
	def __init__(self, cross_section:section.Crosssection, maximum_curvature:float, minimum_curvature:float, 
	strain_position:float, strain_at_position : float, applied_axial_force:float=0.0, maximum_iterations:int=10, axial_force_tolerance:float=5.0, solver:solver.Solver=solver.Newton): 
		"""
		Initialization
		...
		solver : solver.Solver
			used solver (Default: solver.Newton)
		"""
		super().__init__(cross_section, applied_axial_force, maximum_iterations, axial_force_tolerance, solver)
	...
	def initialize_boundary_curvatures(self): 
		for index, curvature in enumerate([self.minimum_curvature, self.maximum_curvature]):
...
class MKappaByConstantCurvature(MKappa): 
	...
	__slots__ = '_cross_section', '_applied_curvature', '_maximum_neutral_axis', '_minimum_neutral_axis',  '_applied_axial_force', '_axial_force_tolerance', '_maximum_iterations', '_computations', '_solver', '_successful', '_computed_cross_section', '_iteration'
	
	def __init__(self, cross_section, applied_curvature:float, maximum_neutral_axis:float, minimum_neutral_axis:float, applied_axial_force, maximum_iterations=10, axial_force_tolerance=5, solver=solver.Newton): 
		"""
		Initialization
	...
	def initialize_boundary_curvatures(self): 
		for index, neutral_axis in enumerate([self.minimum_neutral_axis, self.maximum_neutral_axis]):
			self._iteration = index
...
class MKappaCurvePoints: 
	...
	def add(self, moment:float, curvature:float, neutral_axis:float, crosssection, strain:float, position:float, material:str): 
		self._points.append({
			'moment': moment, 
			'curvature': curvature, 
	...
	def curvature(self, by_moment:float): 
		...
		return general.interpolate(by_moment, self.moment_curvature(point_index), self.moment_curvature(point_index+1))
...
class MKappaCurve: 
	...
	__slots__ = '_cross_section', '_include_positive_curvature', '_include_negative_curvature', '_boundaries', '_positive', '_negative', '_m_kappa_points'
	
	def __init__(self, cross_section:section.Crosssection, include_positive_curvature:bool=True, include_negative_curvature:bool=False): 
		"""
		Initialization
		...
		self._m_kappa_points = MKappaCurvePoints() # [moment, curvature, computed_cross_section]
	...
	def _print_initialization(self) -> str: 
		 return general.print_chapter(['Initialization', '--------------', self.__repr__()])
	...
	def _print_m_kappa_points(self): 
		...
		for point in self.m_kappa_points: 
			text.append('{:13.1f} | {:9.6f} | {:12.2f} | {:8.5f} | {:8.2f} | {}'.format(point['moment'], point['curvature'], point['neutral-axis'], point['strain'], point['position'], point['material']))
		text.append('--------------------------------------------------------------------------')
		return general.print_sections(text)				
	...
	def _get_m_kappa_curve_curvature(self, boundary): 
		return MKappaCurveCurvature(
			...
			minimum_curvature=boundary.minimum_curvature.compute(boundary.maximum_curvature.start.strain, boundary.maximum_curvature.start.position), 
	...
	def _compute_negative_curvature_intermediate(self): 
		for strain_position in self.negative.get_material_points_inside_curvature():
			...
			m_kappa = self._m_kappa(
				position_strain=position, strain_at_position=strain, 
	...
	def _compute_values(self, m_kappa:MKappa, material:str=None): 
		if m_kappa.successful: 
			self._save_values(m_kappa.moment, m_kappa.curvature, m_kappa.neutral_axis, m_kappa.computed_cross_section, m_kappa.strain_at_position, m_kappa.strain_position, material)
	
	def _insert_zero(self): 
		self._save_values(0.0, 0.0, 0.0, None, 0.0, 0.0, '-')
			
	def _m_kappa(self, position_strain:float, strain_at_position:float, maximum_curvature:float, minimum_curvature:float): 
	...
	def _save_values(self, moment, curvature, neutral_axis, computed_cross_section, strain:float=None, position:float=None, material:str=None): 
		self._m_kappa_points.add(moment, curvature, neutral_axis, computed_cross_section, strain, position, material)
...
class MKappaCurveCurvature: 
	...
	__slots__ = '_cross_section', '_maximum_curvature', '_minimum_curvature', '_start_strain', '_start_position', '_m_kappa_failure'
	
	def __init__(self, cross_section, maximum_curvature:float, minimum_curvature, start_strain:float, start_position:float): 
		"""
		Initialization
	...
	def get_material_points_inside_curvature(self) -> list: 
		return self.m_kappa_failure.computed_cross_section.get_material_points_inside_curvature() 
	...
	def _m_kappa(self, strain_position:float, strain_at_position:float, maximum_curvature:float, minimum_curvature:float): 
		return MKappaByStrainPosition(
			self.cross_section, 
...
class MNZeroCurvature: 
	...
	__slots__ = '_cross_section', '_input_section_type', '_input_strain', '_counter_lower_strain', '_counter_upper_strain', '_axial_force_tolerance', '_maximum_iterations', '_solver', '_computations', '_iteration', '_other_strain', '_other_axial_force', '_other_cross_section', '_successful', '_input_cross_section', '_other_cross_section', '_axial_force'
	
	def __init__(self, cross_section:section.Crosssection, input_section_type:str, input_strain:float, counter_lower_strain:float, counter_upper_strain:float, axial_force_tolerance:float=5.0, maximum_iterations:int=10, solver:solver.Solver=solver.Newton): 
		"""
		Initialization
	...
	def _print_other_sections_iterations(self) -> str: 
		...
		for computation in self.computations: 
			text.append('{:4} | {:8.5f} | {:9.2f}'.format(computation['iteration'], computation['strain'], computation['axial-force']))
		text.append('----------------------------')
		return general.print_sections(text)
	...
	def _compute_input_section(self): 
		self._input_cross_section = self._create_crosssection(self.input_sections(), self.input_strain)
		self._axial_force = self.input_cross_section.total_axial_force()
	
	def _compute_other_section(self): 
		self._other_cross_section = self._create_crosssection(self.other_sections(), self.other_strain)
	...
	def _guess_new_strain(self): 
		return self.solver(
			data=self.computations, 
			variable='strain', 
			target='axial-force').compute()
...
class MNZeroCurvatureCurve: 
	...
	__slots__ = '_cross_section', '_section_results', '_axial_force_starting_points', '_m_n_points'
	...
	def _print_section_results_table(self) -> str: 
		return self._print_results_table('Section Results', self._print_section_results())
	
	def _print_section_results(self) -> str: 
		return general.print_sections([self._print_section_result(section_result) for section_result in self.section_results])
	...
	def _print_axial_force_starting_points(self) -> str: 
		return general.print_sections([self._print_section_result(starting_point) for starting_point in self.axial_force_starting_points])
	
	def _print_axial_force_table(self) -> str: 
		return self._print_results_table('Axial force starting points', self._print_axial_force_starting_points())
	...
	def _print_m_n_points_table_content(self) -> str: 
		return general.print_sections([self._print_m_n_point_row(m_n_point) for m_n_point in self.m_n_points])
	...
	def compute_section_axial_forces(self): 
		for section_strain in self._get_section_strains(): 
			self.compute_section_axial_force(strain=section_strain['strain'], section_type=section_strain['section-type'])
	...
	def results_of_section_type(self, section_type:str) -> list: 
		return [section_result for section_result in self.section_results if section_result['section-type'] == section_type]
			
	def _maximum_axial_force(self, section_type:str): 
		return max(self.results_of_section_type(section_type), key=lambda x: x['axial-force'])['axial-force']
	
	def _minimum_axial_force(self, section_type:str): 
		return min(self.results_of_section_type(section_type), key=lambda x: x['axial-force'])['axial-force']
	
	def counter_sections(self, section_type): 
		return self.cross_section.sections_of_type(self.counter_section_type(section_type))
	...
	def find_border_strains(self, axial_force:float, counter_section_type:str): 
		...
		for index in range(len(counter_sections)-1):
			if counter_sections[index]['axial-force'] <= axial_force <= counter_sections[index+1]['axial-force']: 
				return (counter_sections[index]['strain'], counter_sections[index+1]['strain'])
	...
	def collect_axial_force_starting_points(self): 
		for index, section_result in enumerate(self.section_results): 
			counter_section_type = self.counter_section_type(section_result['section-type'])
			if self._is_axial_starting_point(counter_section_type, section_result['axial-force']): 
				self._axial_force_starting_points.append(section_result)
					
	def _is_axial_starting_point(self, counter_section_type:str, axial_force:float) -> bool: 
		if axial_force == 0.0: 
			return False
		elif (-1.) * self._maximum_axial_force(counter_section_type) < axial_force < (-1.) * self._minimum_axial_force(counter_section_type): 
			return True
		else: 
			return False
	...
	def compute_m_n_points(self) -> None: 
		for starting_point in self.axial_force_starting_points: 
			...
			counter_section_type = self.counter_section_type(starting_point['section-type'])
			lower_strain, upper_strain = self.find_border_strains(axial_force*(-1.), counter_section_type)
			m_n_zero_curvature = self._create_mnzerocurvature(
				section_type=starting_point['section-type'], strain=starting_point['strain'], 
				counter_lower_strain=lower_strain, counter_upper_strain=upper_strain)
			self._save_m_n_points(m_n_zero_curvature.resulting_crosssection())
			
	def _create_mnzerocurvature(self, section_type:str, strain:float, counter_lower_strain:float, counter_upper_strain:float): 
