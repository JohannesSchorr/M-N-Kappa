...
class Solver():
	...
	__slots__ = '_data', '_target', '_variable', '_target_value', '_maximum_variable', '_minimum_variable', '_function'
	...
	def _set_maximum_variable(self) -> None: 
		self._maximum_variable = max(self._data, key=lambda x: x[self.variable])[self.variable]
		
	def _set_minimum_variable(self) -> None: 
		self._minimum_variable = min(self._data, key=lambda x: x[self.variable])[self.variable]
...
class Bisection(Solver): 
	...
	__slots__ = '_data', '_target', '_variable', '_target_value', '_maximum_variable', '_minimum_variable'
	
	def compute(self) -> float: 
		print('min over zero:', self._min_over_zero_variable(), '| min under zero:', self._min_under_zero_variable(), '| mean:', 0.5 * (self._min_under_zero_variable() + self._min_over_zero_variable()))
		return 0.5 * (self._min_under_zero_variable() + self._min_over_zero_variable())
	
	def _min_over_zero_variable(self) -> float: 
		return min(self._data_with_target_over_zero(), key=lambda x: x[self.target])[self.variable]
		
	def _min_under_zero_variable(self) -> float: 
		return max(self._data_with_target_under_zero(), key=lambda x: x[self.target])[self.variable] 
...
class Newton(Solver):
	...
	def _set_function(self):
		if len(self.data) > 2: 
			self._function = function.Polynominal(data=self.data, variable=self.variable, target=self.target)
		else: 
			self._function = function.Linear(data=self.data, variable=self.variable, target=self.target)
