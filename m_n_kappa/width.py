...
class StateMeta: 
	...
	def __init__(self, slab_width:float, beam_length:float, mu:float=0.2, sequences:int=10): 
	...
	def _nominator_list(self, position): 
		return [self._nominator(position, sequence) for sequence in range(1, self.sequences)]
	...
	def _determinator_list(self, position):
		return [self._determinator(position, sequence) for sequence in range(1, self.sequences)]
	...
	def b_eff(self, position): 
		return float(
			self.slab_width * ratio_beff_to_b(self, position)
			)
...
class StateMetaDecimal(StateMeta): 
	...
	def sinh(self, value:float) -> decimal.Decimal: 
		...
		"""
		return decimal.Decimal(0.5) * (decimal.Decimal(value).exp() - decimal.Decimal((-1)*value).exp())
...
class MembranStateHarmonicLoadingMultipleWebs(StateMeta): 
	...
	def ratio_beff_to_b(self): 
		...
		return (
			...
			(2./self.pi) * 
			(1./self.b_over_l) * 
			((self.sinh(self.pi * self.b_over_l))**(2.) / 
			(self.pi * self.b_over_l - 3. * self.sinh(self.pi * self.b_over_l) * self.cosh(self.pi * self.b_over_l))))
...
class MembranStateHarmonicLoadingOneWeb(StateMeta): 
	...
	def _A(self) -> float: 
		...
		return (
			((1.+self.mu) * (self.sinh(self.alpha))**(2.) + (1.-self.mu) * (self.alpha**(2.))) / 
	...
	def _B(self) -> float: 
		...
		return (
			#(-1.) * 
			((1.+self.mu) + (1.-self.mu) * (self.cosh(self.alpha))**(2.)) / 
			(self.alpha + self.sinh(self.alpha)*self.cosh(self.alpha)) 
			)
...
class MembranStateHarmonicLoading(DistinguishWebNumber): 
	...
	def __init__(self, slab_width, beam_length, mu:float=0.2, sequences:int=None): 
		self._one_web = MembranStateHarmonicLoadingOneWeb(slab_width, beam_length, mu, sequences)
		self._multiple_webs = MembranStateHarmonicLoadingMultipleWebs(slab_width, beam_length, mu, sequences)
...
class MembranStateLineLoadingMultipleWebs(StateMetaDecimal): 
	...
	def _determinator(self, position:float, sequence:int): #
		...
		return decimal.Decimal(
			...
			self.cos(self.alpha_k(sequence)*position) * 
			(self._A(sequence) * self.cosh(self.alpha_k(sequence)*self.slab_width) + self._B(sequence))
			)
	...
	def _A(self, sequence:int):
		...
		return decimal.Decimal(
			...
			((1+self.mu)*alpha_k*self.cosh(alpha_k)-(1-self.mu)*self.sinh(alpha_k)) / 
			(self.sinh(alpha_k)**(2))
			)
	...
	def _B(self, sequence:int):
		...
		return decimal.Decimal(
			(-1) * 
			decimal.Decimal(1 / (2*sequence-1)) * (1+self.mu) * 
...
class MembranStateLineLoadingOneWeb(StateMetaDecimal): 
	...
	def _determinator(self, position, sequence): 
		...
		return decimal.Decimal(
			(-1)**(sequence-1) * 
			decimal.Decimal(1 / (2*sequence-1)**(2)) * 	
			self.cos(alpha_k) * (self._A(sequence) + 2 * self._B(sequence))
		)
	...
	def _B(self, sequence): 
		...
		alpha_k = self._alpha_k(sequence) * self.slab_width
		return (
			((1-self.mu)+(1+self.mu)*self.cosh(alpha_k)**(2)) / 
			(alpha_k + self.sinh(alpha_k)*self.cosh(alpha_k)) 
			)
...
class MembranStateLineLoading(DistinguishWebNumber): 
	...
	def __init__(self, slab_width, beam_length, mu:float=0.2, sequences:int=55): 
		self._one_web = MembranStateLineLoadingOneWeb(slab_width, beam_length, mu, sequences)
		self._multiple_webs = MembranStateLineLoadingMultipleWebs(slab_width, beam_length, mu, sequences)
...
class MembranStateSingleLoadOneWeb(StateMetaDecimal): 
	...
	def _determinator(self, position, sequence): 
		...
		return (
			decimal.Decimal(1 / (2*sequence-1)) * 
			self.cos(alpha_x)*(self._A(sequence)+2*self._B(sequence))
			)
...
class BendingStateHarmonicLoadingMultipleWebs(StateMeta): 
	...
	def ratio_beff_to_b(self): 
		...
		return (
			...
			(1. / self.b_over_l) * 
			(1. /
			((1.-self.mu)*self._A() - 2.*self.mu*self._B()))
			) 
	...
	def _A(self): 
		...
		return (
			(1./(1.-self.mu)) * 
			(4.+(3.+self.mu)*(1.-self.mu)*self.sinh(self.alpha)**(2.)+(1.-self.mu)**(2.)*self.alpha**(2.)) / 
			((1.-self.mu)*self.alpha-(3.+self.mu)*self.sinh(self.alpha)*self.cosh(self.alpha))
			) 
	...
	def _B(self): 
		...
		return (
			(-1.) * 
			(2.+(3.+self.mu)*self.sinh(self.alpha)**(2.)) / 
			((1.-self.mu)*self.alpha-(3.+self.mu)*self.sinh(self.alpha)*self.cosh(self.alpha))
			)
...
class BendingStateHarmonicLoadingOneWeb(StateMeta): 
	...
	def ratio_beff_to_b(self):
		...
		return (
			(2./self.pi) * 
			(1./self.b_over_l) * 
			((1.-self.mu)*(self.alpha)+(1.+self.mu)*self.sinh(self.alpha) * self.cosh(self.alpha)) / 
			(self.sinh(self.alpha))**(2.)
			)
...
class BendingStateHarmonicLoading(DistinguishWebNumber): 
	...
	def __init__(self, slab_width, beam_length, mu:float=0.2, sequences:int=1): 
		self._one_web = BendingStateHarmonicLoadingOneWeb(slab_width, beam_length, mu, sequences)
		self._multiple_webs = BendingStateHarmonicLoadingMultipleWebs(slab_width, beam_length, mu, sequences)
...
class BendingStateLineLoadingMultipleWebs(StateMetaDecimal): 
	...
	def _determinator(self, position, sequence): 
		...
		return decimal.Decimal(
			(-1)**(sequence-1) * 
			decimal.Decimal(1 / (2*sequence-1)**(2)) * 
			((1-self.mu) * self._A(sequence) * self.cosh(self._alpha_k(sequence)*self.slab_width)+self._B(sequence))
			)
	...
	def _A(self, sequence):
		...
		alpha_k = self._alpha_k(sequence)*self.slab_width
		return (
			(alpha_k*self.cosh(alpha_k)+self.sinh(alpha_k)) / 
			(self.sinh(alpha_k)**(2))
			)
	...
	def _B(self, sequence): 
		...
		alpha_k = self._alpha_k(sequence)*self.slab_width
		return (-1) * (
			((1-self.mu) * alpha_k * self.sinh(alpha_k) - 2*self.mu*self.cosh(alpha_k)) / 
			(self.sinh(alpha_k)))
...
class BendingStateLineLoadingOneWeb(StateMetaDecimal): 
	...
	def _A(self, sequence:int): 
		...
		return decimal.Decimal(
			(1 / (1-self.mu)) * (
			((3+self.mu)*(1-self.mu)*self.cosh(alpha_k)**(2)+(1+self.mu)**(2)+(1-self.mu)**(2)*alpha_k**(2)) / 
			((1-self.mu)*alpha_k-(3+self.mu)*self.sinh(alpha_k)*self.cosh(alpha_k))
			))
...
class BendingStateLineLoading(DistinguishWebNumber): 
	...
	def __init__(self, slab_width:float, beam_length:float, mu:float=0.2, sequences:int=10): 
		self._one_web = BendingStateLineLoadingOneWeb(slab_width, beam_length, mu, sequences)
		self._multiple_webs = BendingStateLineLoadingMultipleWebs(slab_width, beam_length, mu, sequences)
...
class BendingStateSingleLoadOneWeb(StateMetaDecimal): 
	...
	def _determinator(self, position, sequence): 
		...
		return (
			decimal.Decimal(1 / (2*sequence-1)) * 
			self.cos(alpha_k) * ((1-self.mu)*self._A(sequence) + 2*self.mu*self._B(sequence)))
	...
	def _A(self, sequence): 
		...
		return (
			(1 / (1-self.mu)) * 
			((4+(3+self.mu)*(1-self.mu)*self.sinh(alpha_k)**(2)+(1-self.mu)**(2)*alpha_k**(2))) / 
			((1-self.mu)*alpha_k-(3+self.mu)*self.sinh(alpha_k)*self.cosh(alpha_k))
			)
	...
	def _B(self, sequence): 
		...
		alpha_k = self.alpha_k(sequence) * self.slab_width
		return (
			((-1)*(1+self.mu)+(3+self.mu)*self.cosh(alpha_k)**(2)) / 
			((1-self.mu)*alpha_k-(3+self.mu)*self.sinh(alpha_k)*self.cosh(alpha_k))
			)
...
class HarmonicLoadingMultipleWebs(DistinguishState): 
	...
	"""
	def __init__(self, slab_width:float, beam_length:float, mu:float=0.2): 
		self._membran = MembranStateHarmonicLoadingMultipleWebs(slab_width, beam_length, mu)
		self._bending = BendingStateHarmonicLoadingMultipleWebs(slab_width, beam_length, mu)
...
class MultipleWebs(DistinguisLoad): 
	"""
	Composite girder with multiple webs
	"""
	def __init__(self, slab_width:float, beam_length:float, mu:float=0.2): 
		self._single = SingleLoadMultipleWebs(slab_width, beam_length, mu)
