...
def curvature(neutral_axis: float, position: float, strain_at_position : float):
    if neutral_axis == position: 
        raise ZeroDivisionError('Arguments "neutral_axis" and "position" must have different values')
    else: 
        return strain_at_position / (position-neutral_axis)
        
def curvature_by_points(top_edge : float, bottom_edge : float, top_strain: float, bottom_strain : float): 
...
def negative_sign(list_of_lists : list) -> list: 
	return [[(-1)*abs(sublist[0]), (-1)*abs(sublist[1])] for sublist in list_of_lists]
