...
class TestMaterial(unittest.TestCase): 
	...
	stress_strain_list = [[-100., -1.], [-50., -0.6], [0., 0.], [50, 0.5], [150., 2.]]
	section_type = "slab"
	my_material = material.Material(section_type=section_type, stress_strain=stress_strain_list)
	...
	def test_maximum_strain(self): 
		self.assertEqual(self.my_material.maximum_strain, 2.)
		
	def test_minimum_strain(self): 
		self.assertEqual(self.my_material.minimum_strain, -1.)

	def test_strains(self): 
		self.assertEqual(self.my_material.strains, [-1., -0.6, 0., 0.5, 2.])
		
	def test_stresses(self): 
		self.assertEqual(self.my_material.stresses, [-100., -50., 0., 50., 150.])
	
	def test_get_material_index_positive_strain(self): 
		self.assertEqual(self.my_material._get_material_index(strain=1.), 3)
	
	def test_get_material_index_zero(self): 
		self.assertEqual(self.my_material._get_material_index(strain=0.), 2)
	...
	def test_get_intermediate_strains_positive(self): 
		self.assertEqual(self.my_material.get_intermediate_strains(strain_1=1., strain_2=0.), [0.5])
		
	def test_get_intermediate_strain_negative(self): 
		self.assertEqual(self.my_material.get_intermediate_strains(strain_1=-1., strain_2=0.), [-0.6])
		
	def test_get_intermediate_strain_with_two_strains(self): 
		self.assertEqual(self.my_material.get_intermediate_strains(strain_1=-1., strain_2=1.), [-0.6, 0.5])
		
	def test_get_material_stress_1(self): 
		self.assertEqual(self.my_material.get_material_stress(-1.), -100.)
		
	def test_get_material_stress_2(self): 
		self.assertEqual(self.my_material.get_material_stress(.25), 25) 	
		
	def test_get_material_stress_3(self): 
		self.assertEqual(self.my_material.get_material_stress(2.), 150.)
...
class TestConcreteCompressionNonlinear(unittest.TestCase): 
	...
	f_cm = 35.
	concrete = material.Concrete(f_cm=f_cm) 
	compression = material.ConcreteCompressionNonlinear(f_cm, concrete.epsilon_y, concrete.E_cm)
	...
	def test_stress_strains(self): 
		self.assertEqual(self.compression.stress_strain(), [[-0.0, -0.0], [-13.125764641863459, -0.0004370035264271907], [-22.461118693025618, -0.0008396726328503419], [-33.58438351512978, -0.0016793452657006837], [-35.0, -0.002107458997361724], [-31.304785402824145, -0.002803729498680862], [-20.338424138047706, -0.0035]])
...
class TestSteel(unittest.TestCase): 
	...
	def test_stress_strain(self): 
		self.assertListEqual(self.steel.stress_strain, [[-210000., -1.], [0., 0.], [210000, 1.]])
...
class TestSteelBilinear(unittest.TestCase): 
	...
	def test_epsilon_y(self): 
		epsilon_y = self.f_y / 210000.
		self.assertEqual(self.steel.epsilon_y, self.epsilon_y)
		
	def test_stress_strain(self): 
		self.assertEqual(self.steel.stress_strain, [[-500.0, -0.15], [-355.0, -0.0016904761904761904], [-0.0, -0.0], [355.0, 0.0016904761904761904], [500.0, 0.15]])
