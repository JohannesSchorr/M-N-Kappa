...
class TestRectangle(unittest.TestCase): 
	...
	rectangle = geometry.Rectangle(top_edge=top_edge, bottom_edge=bottom_edge, width=width)
	...
	def test_centroid(self): 
		self.assertEqual(self.rectangle.centroid, 0.5 * (self.top_edge + self.bottom_edge))
...
class TestCircle(unittest.TestCase): 
	...
	def test_area(self): 
		self.assertEqual(self.circle.area, self.diameter**(2.)*0.25*3.145)
...
class TestTrapazoidRectangle(unittest.TestCase): 
	...
	trapazoid = geometry.Trapazoid(top_edge=top_edge, bottom_edge=bottom_edge, top_width=top_width, bottom_width=bottom_width)
	...
	def test_width_interception(self): 
		self.assertEqual(self.trapazoid.width_interception, self.rectangle.width_interception)
...
class TestTrapazoidTriangle(unittest.TestCase): 
	...
	def test_centroid(self): 
		self.assertAlmostEqual(self.trapazoid.centroid, 1./3. * self.height)
	...
	def test_width_slope(self): 
		slope = (self.top_width - self.bottom_width) / (self.top_edge - self.bottom_edge)
	...
	def test_split(self): 
		...
		trapazoids = [
			...
			geometry.Trapazoid(point, self.bottom_edge, width_at_point, self.bottom_width)
			]
