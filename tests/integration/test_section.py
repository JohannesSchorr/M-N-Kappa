...
rectangle = geometry.Rectangle(0., 10., 10.)
steel = material.Steel(355.)
...
class TestComputationSectionStrain(unittest.TestCase): 
	...
	def test_edge_strains(self): 
		self.assertEqual(self.computation_section.edges_strain, [self.strain, self.strain])	
		
	def test_edges_stresses(self): 
		self.assertEqual(self.computation_section.edges_stress, [self.stress, self.stress])
	
	def test_axial_force(self): 
		self.assertEqual(self.computation_section.axial_force, self.stress * rectangle.area)
		
	def test_centroid(self): 
		self.assertEqual(self.computation_section.lever_arm(), 0.5 * (rectangle.top_edge + rectangle.bottom_edge))
		
	def test_moment(self): 
		self.assertEqual(self.computation_section.moment(), 0.5 * (rectangle.top_edge + rectangle.bottom_edge) * self.stress * rectangle.area)	
...
class TestComputationSectionCurvature(unittest.TestCase): 
	...
	computation_section = section.ComputationSectionCurvature(my_section, curvature=curvature, neutral_axis=neutral_axis)
	lever_arm = my_section.geometry.top_edge + 1./3. * (my_section.geometry.bottom_edge - my_section.geometry.top_edge)
	axial_force = (-1) * rectangle.area * curvature * (rectangle.bottom_edge - rectangle.top_edge) * steel.E_a * 0.5
	...
	def test_edges_strain(self): 
		top_strain = self.curvature * (my_section.geometry.top_edge - my_section.geometry.bottom_edge)
		self.assertEqual(self.computation_section.edges_strain, [top_strain, 0.0])
		
	def test_edges_stress(self): 
		stresses = [strain * steel.E_a for strain in self.computation_section.edges_strain]
	...
	def test_moment(self): 
		self.assertAlmostEqual(self.computation_section.moment(), self.axial_force * self.lever_arm)
